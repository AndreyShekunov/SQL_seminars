-- 1. Создание БД seminar3
CREATE DATABASE IF NOT EXISTS seminar3;  -- создание БД если она не существует 

-- 2. Подключение к БД seminar3
USE seminar3;
...
SELECT
	id,
    salary,
    CONCAT(firstname, ' ', lastname) AS fullname  -- конкатинация строк, пример: "Вася" + " " + "Петров" = "Вася Петров"
    FROM staff
    ORDER BY salary;  -- автоматически идет сортировка по возрастанию, DESC - по убыванию 
    
-- 4.1 Получим ТОП-5 самых высокооплачиваемых сотрудников в нашей фирме 
SELECT
	id,
    salary,
    CONCAT(firstname, ' ', lastname) AS fullname
    FROM staff
    ORDER BY salary DESC -- DESC - сортировка по убыванию
	LIMIT 5;  -- из выборки получаем первые 5 строк
    
-- 4.2 Выполним сортировку по двум полям 
SELECT
	id,
    salary,
    firstname,
    lastname
FROM staff
ORDER BY lastname DESC, firstname DESC, salary;  -- "Я" -> "A"

-- 5. DISTINCT - для получения уникальных значений 
-- Нужно получить список уникальных фамилий

SELECT DISTINCT lastname  -- выбираем укникальные фамилии 
FROM staff
ORDER BY lastname         -- сортируем в алфавитном порядке 
LIMIT 5;                  -- выведим первых 5 сотрудников

-- 6. GROUP BY - группировка. Позволяет избавиться от дублирующих значений и получить по ним аналитику
-- Для каждой отдельно взятой должности найдем:
	-- 1) среднюю зарплату 
	-- 2) минимальную и максимальную зарплату 
	-- 3) поищем разницу между максимальной и минимальной зарплатой 
	-- 4) количество сотрудников в данной должности 

-- SET sql_mode=(SELECT REPLACE(@@sql_mode, 'ONLY_FULL_GROUP_BY', '')); - штука чтобы убрать ошибку 

SELECT
	post,					  		-- вывод столбца должности 
    ROUND(AVG(salary), 2) AS avg_s, -- поиск средней зарплаты по столбцу salary с округлением до 2 знаков после ","
    ROUND(SUM(salary)) AS sum_s,    -- суммарная зарплата по должности c округлением до целых чисел 
    COUNT(salary) AS count_s, 		-- кол-во сотрудников в данной должности 
    MAX(salary) AS max_s,			-- максимальная зарплата
    MIN(salary) AS min_s,			-- минимальная зарплата
    MAX(salary) - MIN(salary) AS difference  -- разница между максимальной и минимальной зарплатой
FROM staff
WHERE post NOT IN ('Уборщик')  		-- исключаем из выборки должности 'Начальник' и 'Уборщик'. Аналог данной записи: post != 'Начальник' OR post != 'Уборщик'
GROUP BY post     					-- группируем по должностям 
HAVING AVG(salary) > 25000;			-- в полученной выборке интересуют еще записи, где средняя зарплата > 25000 р.

-- !!! оператор WHERE не работает с агрегатными функциями (SUM, AVG, MIN, MAX, COUNT)
-- HAVING идет всегда в паре GROUP BY, если GROUP BY нет, то HAVING работает как WHERE, но правилам хорошего тона
-- HAVING не нужно использовать вместо WHERE. И HAVING работает с агрегатными функциями.

-- 7. База данных из раздела "Проектирование БД" - дополнительный материал
DROP DATABASE IF EXISTS seminar3_dop;
CREATE DATABASE seminar3_dop;
USE seminar3_dop;
...
UPDATE media 
SET created_at = '1991-11-16 08:14:38.000'
WHERE id = 1;
UPDATE media 
SET created_at = '1991-11-16 08:14:38.000'
WHERE id = 3;
UPDATE media 
SET created_at = '1986-12-10 19:34:26.000'
WHERE id = 2;
UPDATE media 
SET created_at = '1986-12-10 19:34:26.000'
WHERE id = 4;
...
-- Создадим архив по месяцам (сколько документов было создано в каждом месяце)
SELECT
	COUNT(id) AS count_media,
    MONTHNAME(created_at) AS month_name, -- 06 - Июнь 
