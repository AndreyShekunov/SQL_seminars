...
-- 1 способ с помощью JOIN и WHERE:
SELECT
	cats.name,
    shops.shopname
FROM shops
...
-- 2 способ с помощью подзапроса:
SELECT
    murzik.name,
    shops.shopname    
FROM (SELECT name, shops_id FROM cats WHERE name = 'Murzik') murzik
JOIN shops
ON shops.id = murzik.shops_id;
...
SELECT 
	analysis.an_name AS название_анализа, 
	analysis.an_price AS цена, 
	DATE(orders.ord_datetime) AS дата
FROM analysis
JOIN orders ON analysis.an_id = orders.ord_an
WHERE DATE(orders.ord_datetime) BETWEEN '2020-02-05' AND DATE_ADD('2020-02-05', INTERVAL 1 WEEK)
ORDER BY дата;
...
WHERE TIMESTAMPDIFF(YEAR, p.birthday, CURDATE()) < 12;  -- функция TIMESTAMPDIFF используется для вычисления  разницы в годах между датой рождения пользователя (p.birthday) и текущей датой (CURDATE())
